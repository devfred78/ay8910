...
#ifndef STANDALONE_AY8910_H
#define STANDALONE_AY8910_H
...
public:
	...
	ay8910_device(psg_type_t psg_type, int clock, int streams, int ioports, int feature = PSG_DEFAULT);
	...
	void set_port_a_read_cb(std::function<u8()> cb) { m_port_a_read_cb = cb; }
	void set_port_b_read_cb(std::function<u8()> cb) { m_port_b_read_cb = cb; }
	void set_port_a_write_cb(std::function<void(u8)> cb) { m_port_a_write_cb = cb; }
	void set_port_b_write_cb(std::function<void(u8)> cb) { m_port_b_write_cb = cb; }
...
private:
	...
	// inlines
	inline u8 tone_enable(int chan) { return BIT(m_regs[AY_ENABLE], chan); }
	...
	inline u8 noise_enable(int chan) { return BIT(m_regs[AY_ENABLE], 3 + chan); }
...
private:
	...
	std::function<u8()> m_port_a_read_cb;
	std::function<u8()> m_port_b_read_cb;
	std::function<void(u8)> m_port_a_write_cb;
