...
Note:
    Requires `numpy`, `sounddevice`, and `lhafile` for full functionality.
...
def play_ym(filename: str, output_wav: str, live_play: bool, engine: str = "cap32") -> None:
    if live_play and (sd is None or np is None):
    ...
    if len(data) > 6 and b'-lh' in data[2:6]:
        if lhafile is None:
            ...
            print("Please install it by running: pip install ay8910_wrapper[tools]", file=sys.stderr)
            return
